<x-layouts::app.owner>
    <div class="p-4 sm:p-6 lg:p-8 max-w-[1100px] mx-auto space-y-6">
        <div>
            ...
            <p class="mt-1 text-sm text-slate-500">Customer conversations scoped to their quotes.</p>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            @if($conversations->isEmpty())
                <div class="p-12 text-center text-slate-500">
                    ...
                    <p class="text-sm mt-1">Customers will appear here once they start a chat from a quote.</p>
                </div>
            @else
                <ul class="divide-y divide-slate-100">
                    @foreach($conversations as $conv)
                        <li>
                            <a href="{{ route('owner.chat.show', $conv) }}"
                               class="flex items-center gap-4 p-5 hover:bg-slate-50 transition-colors">
                                <div class="w-11 h-11 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-700 font-semibold text-sm shrink-0">
                                    {{ strtoupper(substr($conv->customer->first_name ?? 'C', 0, 1)) }}{{ strtoupper(substr($conv->customer->last_name ?? '', 0, 1)) }}
                                </div>
                                ...
                                @if(($conv->unread ?? 0) > 0)
                                    <span class="shrink-0 inline-flex items-center justify-center min-w-[24px] h-6 px-2 rounded-full bg-indigo-100 text-indigo-700 text-xs font-semibold">
                                        {{ $conv->unread }}
                                    </span>
                                @endif
                            </a>
