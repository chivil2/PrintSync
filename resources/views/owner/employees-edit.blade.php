<x-layouts::app.owner>
    <div class="p-6">
        <x-employee-banner title="Edit Employee" subtitle="Update employee information" />

        <div>
            <form action="{{ route('owner.employees.update', $employee) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="rounded-xl border border-zinc-200 bg-white shadow-sm p-6 flex items-center gap-6">
                    <div class="flex-shrink-0">
                        @if($employee->profile_photo_path)
                            <img src="{{ asset('storage/' . $employee->profile_photo_path) }}" alt="{{ $employee->first_name }} {{ $employee->last_name }}" class="w-24 h-24 rounded-full object-cover border-4 border-zinc-200">
                        @else
