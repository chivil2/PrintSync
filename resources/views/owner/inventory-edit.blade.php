<x-layouts::app.owner>
    <div class="p-4 sm:p-6 lg:p-8 max-w-[1600px] mx-auto" x-data="{ isLoading: false, handleSubmit() { this.isLoading = true; this.$refs.form.submit(); } }">
        <!-- Header -->
        <div class="bg-emerald-600 rounded-lg px-6 py-4 mb-6">
            <h1 class="text-xl font-bold text-white">Edit Inventory Item</h1>
            <p class="text-emerald-100 text-sm">Update item information</p>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-slate-300 p-6">
            ...
            <form x-ref="form" action="{{ route('owner.inventory.update', $item->id) }}" method="POST" class="space-y-6" @submit.prevent="handleSubmit">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- Item ID (Read-only) -->
                    <div>
                        <label class="block text-sm font-medium text-slate-500 mb-2">Item ID</label>
                    ...
                    <div>
                        ...
                        <input type="number" id="min_stock_level" name="min_stock_level" value="{{ old('min_stock_level', $item->min_stock_level) }}" min="0" required
                            class="w-full px-4 py-2 text-sm border border-slate-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" id="unit_price" name="unit_price" value="{{ old('unit_price', $item->unit_price) }}" step="0.01" min="0" required
                    ...
                    <div>
                        ...
                        <input type="text" id="supplier" name="supplier" value="{{ old('supplier', $item->supplier) }}"
                            class="w-full px-4 py-2 text-sm border border-slate-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
                ...
                <div class="flex items-center justify-end gap-4 pt-4 border-t border-slate-200">
                    ...
                    <button type="submit" :disabled="isLoading"
                        ...
                        <span x-text="isLoading ? 'Updating...' : 'Update Item'"></span>
                    </button>
                </div>
            </form>
        </div>
