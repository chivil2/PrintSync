<x-layouts::app.owner>
    <div class="p-4 sm:p-6 lg:p-8 max-w-[1600px] mx-auto" x-data="reportsData()" x-init="initReports()">
        @php
            ...
            $monthlyRevenue = [120000, 150000, 180000, 210000, 250000, 220000];
            $monthlyOrders = [45, 55, 65, 78, 92, 85];
            $chartColors = ['#3b82f6', '#a855f7', '#f97316', '#10b981', '#ec4899', '#f59e0b', '#06b6d4', '#8b5cf6', '#ef4444', '#14b8a6'];
        @endphp
        ...
        <div class="bg-white/95 rounded-[24px] sm:rounded-[28px] p-5 sm:p-7 shadow-xl shadow-blue-950/10 border border-white/70 mb-6">
            ...
            <div class="h-[280px] mt-6">
                <svg viewBox="0 0 800 280" class="w-full h-full">
                    ...
                    <!-- X-axis labels with tooltips -->
                    <text x="50" y="260" font-size="12" fill="#94a3b8" text-anchor="middle"><title>₱120,000</title>Jul</text>
                    <text x="170" y="260" font-size="12" fill="#94a3b8" text-anchor="middle"><title>₱150,000</title>Aug</text>
                    <text x="290" y="260" font-size="12" fill="#94a3b8" text-anchor="middle"><title>₱180,000</title>Sep</text>
                    <text x="410" y="260" font-size="12" fill="#94a3b8" text-anchor="middle"><title>₱210,000</title>Oct</text>
                    <text x="530" y="260" font-size="12" fill="#94a3b8" text-anchor="middle"><title>₱250,000</title>Nov</text>
                    <text x="650" y="260" font-size="12" fill="#94a3b8" text-anchor="middle"><title>₱220,000</title>Dec</text>
                </svg>
            </div>
        </div>
        ...
        <div class="bg-white/95 rounded-[24px] sm:rounded-[28px] p-5 sm:p-7 shadow-xl shadow-blue-950/10 border border-white/70 mb-6">
            ...
            <div class="h-[220px]">
                <svg viewBox="0 0 800 220" class="w-full h-full">
                    ...
                    <!-- Dots with tooltips -->
                    <circle cx="50" cy="160" r="4" fill="#a855f7" stroke="white" stroke-width="2"><title>₱120,000</title></circle>
                    <circle cx="170" cy="140" r="4" fill="#a855f7" stroke="white" stroke-width="2"><title>₱150,000</title></circle>
                    <circle cx="290" cy="110" r="4" fill="#a855f7" stroke="white" stroke-width="2"><title>₱180,000</title></circle>
                    <circle cx="410" cy="80" r="4" fill="#a855f7" stroke="white" stroke-width="2"><title>₱210,000</title></circle>
                    <circle cx="530" cy="50" r="4" fill="#a855f7" stroke="white" stroke-width="2"><title>₱250,000</title></circle>
                    <circle cx="650" cy="70" r="4" fill="#a855f7" stroke="white" stroke-width="2"><title>₱220,000</title></circle>
                    <circle cx="750" cy="90" r="4" fill="#a855f7" stroke="white" stroke-width="2"><title>₱195,000</title></circle>
            ...
            <div class="h-[140px]">
                <svg viewBox="0 0 800 140" class="w-full h-full">
                    ...
                    <rect x="70" y="60" width="40" height="40" fill="#f97316" rx="6"><title>Total Orders: 45</title></rect>
                    <rect x="190" y="50" width="40" height="50" fill="#f97316" rx="6"><title>Total Orders: 55</title></rect>
                    <rect x="310" y="40" width="40" height="60" fill="#f97316" rx="6"><title>Total Orders: 65</title></rect>
                    <rect x="430" y="30" width="40" height="70" fill="#f97316" rx="6"><title>Total Orders: 78</title></rect>
                    <rect x="550" y="20" width="40" height="80" fill="#f97316" rx="6"><title>Total Orders: 92</title></rect>
                    <rect x="670" y="35" width="40" height="65" fill="#f97316" rx="6"><title>Total Orders: 85</title></rect>
        ...
        <div class="bg-white/95 rounded-[24px] sm:rounded-[28px] p-5 sm:p-7 shadow-xl shadow-blue-950/10 border border-white/70 mb-6">
            ...
            @php
                $totalProductQty = $topProducts->sum('total_qty');
                $circumference = 741;
                $cumOffset = 0;
                    ...
                    <div class="relative w-[280px] h-[280px] rounded-[32px] bg-[#f5ede3] p-4 shadow-inner">
                        <svg viewBox="0 0 280 280" class="w-full h-full">
                            @foreach($topProducts as $i => $product)
                                @php
                                    $pct = $totalProductQty > 0 ? ($product->total_qty / $totalProductQty) * 100 : 0;
                                    $segLen = max(round($pct / 100 * $circumference), 1);
                                    $remaining = $circumference - $segLen;
                                @endphp
                                <circle cx="140" cy="140" r="118" fill="none" stroke="{{ $chartColors[$i % count($chartColors)] }}" stroke-width="40" stroke-dasharray="{{ $segLen }} {{ $remaining }}" stroke-dashoffset="-{{ $cumOffset }}" transform="rotate(90 140 140)">
                                    <title>{{ $product->item_name }}: {{ number_format($pct, 1) }}%</title>
                                </circle>
                                @php $cumOffset += $segLen; @endphp
                            @endforeach
                        </svg>
                        <div class="absolute inset-0 flex flex-col items-center justify-center">
                            <div class="text-3xl font-bold text-slate-900">{{ $topProductCount }}</div>
                ...
                <div class="space-y-2">
                    @foreach($topProducts as $i => $product)
                        ...
                        <div class="flex items-center gap-3">
                            <div class="w-32 text-xs text-slate-700 truncate text-right" title="{{ $product->item_name }}">{{ $product->item_name }}</div>
                            <div class="flex-1 h-5 bg-slate-100 rounded-full overflow-hidden">
                                <div class="h-full rounded-full" style="width: {{ $pct }}%; background-color: {{ $chartColors[$i % count($chartColors)] }}">
                                    <title>{{ $product->item_name }}: {{ number_format($pct, 1) }}%</title>
                                </div>
                            </div>
                            <div class="text-xs w-12 text-right font-medium">{{ number_format($pct, 1) }}%</div>
                        </div>
                    @endforeach
                </div>
            </div>
