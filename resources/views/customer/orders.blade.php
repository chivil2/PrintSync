...
<div class="max-w-[1280px] mx-auto px-6 py-8 lg:px-8 lg:py-8 space-y-8" x-data="{ 
    ...
    <div x-show="filteredOrders.length > 0" class="flex flex-col gap-3">
        <template x-for="order in filteredOrders" :key="order.id">
            <div class="bg-white rounded-xl border border-gray-100 p-4 flex items-center gap-4 hover:shadow-md hover:border-gray-200 transition-all">
                <a :href="'/customer/orders/' + order.id" class="flex items-center gap-4 flex-1 min-w-0">
                    ...
                    <div class="text-right text-sm flex-shrink-0">
                        ...
                        <div class="font-extrabold text-[15px] text-gray-900" x-text="order.quote_total ? '₱' + parseFloat(order.quote_total).toFixed(2) : '₱500'"></div>
                    </div>
                </a>
