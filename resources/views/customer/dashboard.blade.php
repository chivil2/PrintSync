...
<div class="max-w-[1280px] mx-auto px-5 py-6 lg:px-8 lg:py-7 space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <a href="{{ route('customer.orders') }}" class="bg-white border {{ $toPayCount > 0 ? 'border-amber-200 ring-2 ring-amber-100' : 'border-slate-100' }} p-5 rounded-3xl card-shadow transition-all cursor-pointer hover:shadow-lg">
            ...
            <div class="text-3xl font-bold text-slate-900">₱{{ number_format($toPayTotal, 2) }}</div>
            <div class="text-xs text-slate-500 mt-0.5">To Pay <span class="font-semibold text-amber-600">({{ $toPayCount }})</span></div>
        </a>
    </div>
