...
<div class="flex-1 flex flex-col" x-data="clock()" x-init="startClock()">
    <div class="px-6 lg:px-8 py-8 flex-1 overflow-y-auto">
        ...
        <div class="bg-gradient-to-r from-orange-500 to-blue-600 rounded-3xl p-8 text-white relative overflow-hidden mb-8">
            ...
            <div class="relative z-10 flex items-center justify-between gap-2">
                <div class="min-w-0 flex-1">
                    <h1 class="text-3xl lg:text-4xl font-bold truncate">Hey, {{ auth()->user()->first_name }}!</h1>
                    <p class="text-orange-100 text-base mt-1">Here&rsquo;s your task overview</p>
                </div>
        ...
        <div class="grid grid-cols-4 gap-4 mb-8">
            <a href="{{ route('employee.jobs') }}" class="bg-white border border-slate-100 p-5 rounded-3xl shadow-sm transition-all cursor-pointer hover:shadow-md">
                <div class="bg-emerald-50 w-12 h-12 rounded-2xl flex items-center justify-center mb-4 text-emerald-600">
            ...
            <a href="{{ route('employee.jobs') }}" class="bg-white border border-slate-100 p-5 rounded-3xl shadow-sm transition-all cursor-pointer hover:shadow-md">
                <div class="bg-orange-50 w-12 h-12 rounded-2xl flex items-center justify-center mb-4 text-orange-600">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('employee.jobs') }}" class="bg-white border border-slate-100 p-5 rounded-3xl shadow-sm transition-all cursor-pointer hover:shadow-md">
                <div class="bg-amber-50 w-12 h-12 rounded-2xl flex items-center justify-center mb-4 text-amber-600">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                </div>
        ...
        <div class="flex items-center justify-between mb-4">
            ...
            @if ($recentJobs->count() > 0)
                <a href="{{ route('employee.jobs') }}" class="text-sm font-medium text-orange-500 hover:text-orange-600 transition-colors">View all</a>
            @endif
        </div>
                        ...
                        <tbody>
                            @foreach ($recentJobs as $job)
                                ...
                                <tr class="border-b border-slate-50 last:border-none hover:bg-slate-50/50 transition-colors group">
                                    <td class="py-4 px-6 font-semibold text-slate-900 text-base">{{ $job->customer->name ?? 'N/A' }}</td>
                                    <td class="py-4 px-6 text-slate-600 text-sm">{{ $job->name }}</td>
                                    <td class="py-4 px-6">
    ...
    {{-- Footer --}}
    <div class="border-t border-slate-100 px-8 py-4 flex items-center justify-between text-xs text-slate-400">
