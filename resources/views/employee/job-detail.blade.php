                    ...
                    <div class="bg-slate-50 rounded-3xl p-6 mb-6 border border-slate-100">
                        <div class="grid grid-cols-4 gap-6">
                            <div>
                                ...
                                <p class="text-slate-900 font-bold mt-1 text-base">{{ $job->name }}</p>
                            </div>
                            <div>
                                <span class="text-slate-400 text-xs font-medium">Type</span>
                                <p class="text-slate-900 font-bold mt-1 text-base">{{ ucfirst($job->type) }}</p>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <p class="text-slate-900 font-bold mt-1 text-base">{{ $job->created_at->format('M d, Y') }}</p>
                            </div>
                            @if ($job->deadline)
                                <div>
                                    <span class="text-slate-400 text-xs font-medium">Deadline</span>
                                    <p class="text-slate-900 font-bold mt-1 text-base">{{ $job->deadline->format('M d, Y') }}</p>
                                </div>
                            @endif
                        </div>
                        @if ($job->description)
                            <div class="mt-5 pt-5 border-t border-slate-200">
                                ...
                                <p class="text-slate-700 mt-1 text-base">{{ $job->description }}</p>
                            </div>
                        @endif
                        @if ($job->notes)
                            <div class="mt-5 pt-5 border-t border-slate-200">
                                <span class="text-slate-400 text-xs font-medium">Notes</span>
                                <p class="text-slate-700 mt-1 text-base">{{ $job->notes }}</p>
                            </div>
                        @endif
                        <div class="mt-5 pt-5 border-t border-slate-200">
                            <span class="text-slate-400 text-xs font-medium">Customer</span>
                            <p class="text-slate-900 font-bold mt-1 text-base">{{ $job->customer->name ?? 'N/A' }}</p>
